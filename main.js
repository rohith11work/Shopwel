    ...
    prods.forEach(p => {
      ...
      if(isOut) {
        ...
        const mrpHtml = hasDiscount 
          ? `<span style="text-decoration:line-through;color:#991b1b;font-size:11px;margin-right:6px;vertical-align:middle;">₹${p.mrp.toFixed(2)}</span>` 
          : '';
        label.innerHTML = `
          ${p.name} 
          <span style="font-size:13px;display:block;margin-top:4px;">
            ${mrpHtml}<span style="color:#137333;background:#e6f4ea;border:1px solid rgba(19,115,51,0.25);padding:1px 5px;border-radius:4px;font-weight:bold;font-size:12px;display:inline-block;vertical-align:middle;">₹${p.price.toFixed(2)}</span>
          </span>`;
      }
...
function renderImpulseUpsells() {
  ...
  impulseProds.forEach(p => {
    ...
    const hasDiscount = p.mrp && p.mrp > p.price;
    const mrpHtml = hasDiscount 
      ? `<span style="text-decoration:line-through;color:#991b1b;font-size:10px;margin-right:4px;vertical-align:middle;">₹${p.mrp.toFixed(2)}</span>` 
      : '';
    card.innerHTML = `
      ...
      <div class="cm-im-info">
        ...
        <span class="cm-im-price">${mrpHtml}<span style="color:#137333;background:#e6f4ea;border:1px solid rgba(19,115,51,0.25);padding:1px 4px;border-radius:4px;font-weight:bold;font-size:11px;display:inline-block;vertical-align:middle;">₹${p.price.toFixed(2)}</span></span>
      </div>
